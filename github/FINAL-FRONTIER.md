...
3. Complete the remaining live gates: legitimate second-human approval/revocation, signed merge_group delivery with an eligible queued fixture inside the unchanged installation scope, and organization-only ruleset enforcement. Development-App delivery and scoped reconciliation now passed using the existing installation; see [App resume](validation/final-frontier-l3-2026-09-19/app-resume.md). Distinct-account approval/dismissal behavior now passed in the [reviewer resume](validation/final-frontier-l3-2026-09-19/reviewer-resume.md). Both accounts have the same operator, so human independence remains NOT_PROVEN. Reviewer permission loss also [passed under explicit authorization](validation/final-frontier-l3-2026-09-19/permission-resume.md); Write access was restored and verified. Organization-ruleset enforcement and signed merge_group remain the outstanding live-provider blockers.
...
Second-account functional approval acceptance passed eight assertions, with six captured receipts replayed. Head movement and explicit reviewer dismissal removed current approval eligibility; fresh approval restored VERIFIED only with all required claims proven. The synthetic PR was closed and its prior protection restored. Human independence and the remaining live/production gates are not claimed.

B03 permission-loss acceptance passed four proof/currentness assertions and replayed three receipts. Confirmed Read access invalidated only the required approval claim and blocked current use of the old VERIFIED receipt. Confirmed Write restoration recovered VERIFIED with the same head and review. The fixture was closed and all temporary settings restored; no product defect or code change resulted.
