...
async function run(){
 ...
 const root=fs.mkdtempSync(path.join(os.tmpdir(),"mp-uid-acceptance-")),credentials=fs.mkdtempSync(path.join(os.tmpdir(),"mp-root-credentials-")),proof=65534,policy=65533,checks=[];let child;
 ...
 try{
  ...
  const refresh=require("./refresh-credentials"),credentialDir=path.join(credentials,"policy"),credentialFile=path.join(credentialDir,"key.pem");
  fs.chmodSync(credentials,0o711);refresh.secureDirectory(credentialDir,policy,0o710);refresh.atomic(credentialFile,"isolated-policy-fixture",policy,policy);
  const staging=as(policy,"try{require('fs').symlinkSync(process.argv[1],process.argv[2]);process.exit(2)}catch(e){process.exit(e.code==='EACCES'?0:3)}",[primary,credentialFile+".tmp"]);
  assert(staging.status===0,"ROOT_CREDENTIAL_DIRECTORY_WRITABLE");checks.push("companion-cannot-replace-privileged-credential-staging");
  const ownRead=as(policy,"process.exit(require('fs').readFileSync(process.argv[1],'utf8')==='isolated-policy-fixture'?0:2)",[credentialFile]);assert(ownRead.status===0,"WORKLOAD_CREDENTIAL_READ_FAILED");
 ...
 }finally{if(child?.pid&&child.exitCode===null){child.kill("SIGTERM");await new Promise(r=>child.once("exit",r));}fs.rmSync(root,{recursive:true,force:true});fs.rmSync(credentials,{recursive:true,force:true});}
}
