...
Prepared 2026-09-20. **Not deployed, not published, no resources purchased or scheduled.** Validated product candidate remains `7953ecdbd849c8afe0379bfec1b4a61133292c62`; policy code digest `b770fe7f5dac86730095e768e4c3c4b4e1856eae909b3757acfc756069e47b76`. Documentation commits do not change that identity. The [accepted signed queue report](../validation/final-frontier-l3-2026-09-19/signed-queue-final.md) remains the live proof record.

The [least-privilege investigation and Enhanced Policy Proof design](../validation/final-frontier-l3-2026-09-19/least-privilege.md) fix the production default at Administration Read. The optional credential integration/UI and positive GitHub coverage aggregate adapter are unimplemented. A future implementation changes the product candidate and needs its own acceptance. `7953ecd` cannot be represented as shipping that mode; its positive signed lab run used an explicitly temporary injected Write credential.
...
| --- | --- | --- |
| Optional Enhanced Policy Proof installation | Approve the proposed companion policy-reader App boundary, account/name and selected test repositories before registration. Confirm customer consent presentation. Primary App remains Read. No org Write is included. | Register only after authorization; implement dedicated policy-client routing/opt-in, then issue a new candidate and validate it. Local design is ready; the companion and shipped integration do not yet exist. |
...
Use an exact, clean checkout of `7953ecd` for this candidate. Documentation HEAD is later and will correctly fail `release-preflight` if supplied as that product SHA. A new Enhanced implementation requires replacing every release pin below with the newly accepted SHA. Do not relabel new code as `7953ecd`.
...
Proposed operating contract for owner review: encrypted independent backups, no automatic historical receipt deletion, daily restore verification and alerts for missing backup/capacity/write errors. Owner must select retention, RPO/RTO and any costs before activation; there is no claimed recovery SLA today. Use a consistent snapshot or quiesce the single writer before copying the complete set. Stop the old writer before starting a restore. A crash may leave `server.lock`; verify the old process is dead and preserve evidence before operator removal. Never run two writers or clear a live lock automatically.
...
Proposed cadence: weekly Sunday 14:00 UTC on the owner's chosen non-production scheduler, plus a manual run after a relevant GitHub schema/permission change. Single concurrency; one bounded run, no unlimited retries; 20-minute scheduler cap around the existing observer's maximum 480 provider requests and 30 fixtures. Respect rate-limit backoff and mark timeout/permission/collection failures UNAVAILABLE, never a passing baseline. The schedule is a proposal, not an activated job.
...
No runtime or test file changed for this packet. Accepted evidence is reused: GitHub 331/331 (local and Linux), 1,000 model seeds with 17 invariants, static reconstruction 20/20, CLI 26/report 10 and one clean factory 37/37; exact candidate signed queue 13/13 with independent portable replay. These are prior executed results on the recorded candidate, not new tests claimed by writing this document. This turn added read-only provider probes, schema comparison, real backup/restore preparation and document/configuration checks.

Required pre-addendum §43 code accounting remains [22 items](../validation/final-frontier-l3-2026-09-19/charter-coverage.md). Positive coverage collection and the newly designed Enhanced customer credential integration/UI remain unimplemented. Optional log INFER, the alternative OIDC authentication branch and later Quint work remain explicitly absent as previously scoped. Production host/IAM/backup/scheduler configuration and external trust/root publication remain unexecuted owner/environment operations. **Level 3 is open.**
