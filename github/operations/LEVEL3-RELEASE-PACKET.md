...
| --- | --- | --- |
| Optional Enhanced Policy Proof installation | Private ohcaygo-owned development registration is authorized for the queue fixture only; GitHub access verification remains. Production registration is separately reserved. Primary App remains Read. No org Write is included. | Enrollment/currentness/IPC implementation and Linux isolation tests passed on 838ed8f. Register/install the authorized companion, then earn fresh provider and signed queue acceptance on that exact candidate. |
