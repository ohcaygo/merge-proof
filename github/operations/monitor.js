...
function inspect(config,now=Date.now()){
 ...
 try{const data=JSON.parse(fs.readFileSync(config.stateDir+"/state.json")).github;assert(data,"STATE_UNAVAILABLE");if(data.operatorLogHealth==="CHECKPOINT_UNAVAILABLE")alarm("SIGNING_OR_CHECKPOINT_UNAVAILABLE");if(data.permissionHealth&&Object.values(data.permissionHealth).some(v=>v?.state==="UNAVAILABLE"))alarm("GITHUB_PERMISSION_UNAVAILABLE","WARNING");}catch{alarm("PROOF_STATE_UNAVAILABLE");}
