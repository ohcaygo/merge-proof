...
function inspect(config,now=Date.now()){
 ...
 const result={at:new Date(now).toISOString(),state:"HEALTHY",alarms:[],recoveryBackupAgeSeconds:null,usedPercent:null,externalHealth:config.externalHealthFile?{state:"NOT_CHECKED"}:notConfigured};
...
async function publish(config){require("./authority").environment(config);const out=await inspectExternal(config),aws=require("./aws").client(config.aws);await aws.checkIdentity();await aws.call("cloudwatch","put-metric-data",["--namespace","MergeProof","--metric-data",JSON.stringify([{MetricName:"RecoveryBackupAgeSeconds",Value:out.recoveryBackupAgeSeconds??86400,Unit:"Seconds"},{MetricName:"StorageUsedPercent",Value:out.usedPercent??100,Unit:"Percent"},{MetricName:"CriticalHealth",Value:out.state==="CRITICAL"?1:0,Unit:"Count"}])]);return out;}
...
module.exports={inspect,publish,inspectExternal,serviceReady,backupReady};
