...
Prepared 2026-09-20 for owner decision. **Recommendation only: no account, App, resource, paid plan, deployment, publication or schedule was created or changed.** This makes the choices in the [release acceptance packet](LEVEL3-RELEASE-PACKET.md) concrete; it does not replace its acceptance requirements.
...
The repository currently documents a dedicated $12/month DigitalOcean origin, Debian 13, Nginx and a single Node writer. This proposal **changes that hosting provider** to obtain short-lived native AWS runtime identity. It preserves the product URL, Pages project, Node service, billing authority, state model, exact Git reconstruction and proof semantics. The move is a specific owner decision, not an already approved or performed change. The documented deployment is the basis for this recommendation; production was not inspected this turn.

## Product and permission boundary

Last accepted signed queue-to-landed product: `7953ecdbd849c8afe0379bfec1b4a61133292c62`. New local product candidate: `5fd27e130a1bd098f89ba3f30d07bf27d1fd5b3b`, which adds a companion reader and collector injection point. Its [validation record](../validation/enhanced-policy-boundary-2026-09-20/README.md) reports local results separately from prior live acceptance. The new candidate has not completed a fresh live signed queue journey.
...
Production integration still must keep the companion private key and token in a dedicated process/OS identity. The normal proof service receives normalized provider observations, never that secret or a general authenticated client. The narrow IPC must bind authenticated customer consent and the existing primary-installation/repository authorization to immutable IDs; reject caller-selected URLs; record relevant provenance; and invalidate/recollect dependent claims on consent, installation or policy change. Register no customer-facing mode until enrollment, UI, revocation, currentness and isolation tests pass. The new library is not a completed enrollment API or an OS sandbox.
...
**Selected:** A new isolated **GitHub Team** organization, proposed name `ohcaygo-merge-proof-validation`, with **two seats: Ryan and one named independent human**. Do not upgrade the production `ohcaygo` organization. Strict minimum for active org branch/tag rules is Team with one licensed owner; the recommended second seat enables genuinely independent human approval/revocation acceptance.
