...
function walk(root,visit,base="",visitDirectory=()=>{}){
 ...
 for(const name of fs.readdirSync(path.join(root,base)).sort()){
  ...
  if(st.isDirectory()){visitDirectory(relative);walk(root,visit,relative,visitDirectory);}else{assert(st.isFile(),"BACKUP_SPECIAL_FILE_DENIED");visit(relative);}
 }
}
function directories(manifest){
 const rows=manifest.directories||[];
 assert(Array.isArray(rows)&&new Set(rows).size===rows.length&&rows.every(p=>safe(p)&&/^mirrors\/[1-9][0-9]*(?:\/|$)/.test(p)),"BACKUP_DIRECTORY_INVALID");return rows;
}
function restoreDirectories(root,manifest){
 assert(path.isAbsolute(root)&&fs.lstatSync(root).isDirectory(),"BACKUP_DIRECTORY_INVALID");
 for(const relative of directories(manifest)){let dir=root;for(const part of relative.split("/")){dir=path.join(dir,part);if(!fs.existsSync(dir))mkdir(dir);assert(fs.lstatSync(dir).isDirectory(),"BACKUP_SYMLINK_DENIED");}}
}
...
function verify(directory){
 ...
 for(const relative of directories(manifest)){let dir=directory;for(const part of relative.split("/")){dir=path.join(dir,part);assert(fs.lstatSync(dir).isDirectory(),"BACKUP_DIRECTORY_INVALID");}}
...
function create(config){
 ...
 try{
  ...
  const capturedAt=new Date().toISOString(),stateBytes=fs.readFileSync(path.join(config.stateDir,"state.json")),files=[],retainedDirectories=[];
  ...
  const retainDirectory=relative=>{mkdir(path.join(config.output,relative));retainedDirectories.push(relative);};
  for(const id of mirrors){retainDirectory("mirrors/"+id);walk(path.join(config.mirrorRoot,id),relative=>copy("mirrors/"+id+"/"+relative,path.join(config.mirrorRoot,id,relative)),"",relative=>retainDirectory("mirrors/"+id+"/"+relative));}
  const manifest={schema:"urn:merge-proof:private-backup:1",sourceCommit:config.sourceCommit,at:capturedAt,stateDigest:digest(stateBytes),files,directories:retainedDirectories};
...
function restore(config){
 ...
 restoreDirectories(config.destination,checked.manifest);
...
module.exports={create,verify,restore,digest,digestFile,restoreDirectories};
