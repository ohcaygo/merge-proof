...
function prove(c, options = {}) {
  c = require("./rules").project(structuredClone(c));
  ...
  const coverage = rules.coverage.map(rule => require("./coverage").evaluate(c, rule, ci));
  const gaps = [];
  ...
  gaps.push(...coverage.filter(x => x.state !== "TRUE").map(x => x.reason));
  ...
  for (const row of ci.required) {
    if (row.producerBound && !gaps.includes("TARGET_BINDING_MISMATCH"))
      for (const p of row.executionProtection.filter(p => p.state === "FALSE"))
        violations.push({ claim: "CI_EXECUTED", reason: p.reason, checkId: row.checkId });
    if (row.producerBound && !gaps.includes("TARGET_BINDING_MISMATCH") &&
  ...
  if (!gaps.some(x => ["TARGET_BINDING_MISMATCH", "CURRENT_MERGE_GROUP_SELECTION_UNAVAILABLE", "CANDIDATE_TREE_UNAVAILABLE"].includes(x)))
    for (const row of coverage.filter(x => x.state === "FALSE"))
      violations.push({ claim: row.name, reason: row.reason, rulesetId: row.rulesetId });
  ...
  return {
    ...
    claims: require("./claims").describe(c, rules, ci, approval, violations, gaps, coverage),
    verdict,
    violations,
...
function freshness(receipt, current) {
  ...
  const { changed, unavailable, claims, changedClaims, claimDetails } = require("./bindings").compare(receipt.evidence, current);
  return {
    state:
