...
function identity(i) {
  const { githubMergeable, githubMergeState, mergeCommitSha, ...bound } = i;
  return bound;
}
function claims(c) {
...
function compare(previous, current) {
  ...
  return { changed, unavailable, claims: Object.fromEntries(Object.keys(after).map(k => [k, {
    binding: after[k], state: changed.includes(k) ? "STALE" : unavailable.includes(k) ? "UNAVAILABLE" : "CURRENT",
  }])) };
}
module.exports = { claims, fingerprints, compare, identity, opinionated };
