...
function requirements(rules) {
  ...
  for (const r of rules.active.value) {
    ...
    if (r.type === "required_status_checks") {
      ...
      out.coverage.push({ rulesetId: r.ruleset_id ?? null, parameters: p || null, state: valid ? "RECOGNIZED" : "MALFORMED" });
  ...
  return client.observe(async () => {
    ...
    for (const row of rows) {
      ...
      out.push({ id: p.id, source: p.source, sourceType: p.source_type, enforcement: p.enforcement,
        conditions: p.conditions || {}, rules: p.rules });
    }
...
function applies(p, workflowPath) {
  ...
  return included.includes(null) || excluded.includes(null) ? null : true;
}
function executionPolicyBinding(c, jobs) {
  const p = c.rules.executionProtections;
  if (p?.state !== "AVAILABLE") return { availability: "UNKNOWN", reason: p?.reason || "NOT_OBSERVED" };
  return { availability: "OBSERVED", policies: p.value.filter(p => p.enforcement !== "disabled" && jobs.some(j => applies(p,j.workflowPath) !== false)) };
}
function executionPolicy(c, job) {
  const policies = c.rules.executionProtections;
  if (policies?.state !== "AVAILABLE") return { state: "UNKNOWN", reason: "EXECUTION_PROTECTIONS_UNAVAILABLE", required: false };
    ...
    for (const rule of p.rules) {
      if (rule.type === "restrict_action_events") {
        ...
        for (const actor of [job.actor, job.triggeringActor]) {
          if (!actor || !actors.some(a => ["User", "Bot"].includes(a.type) && a.type === actor.type && a.id === actor.id))
...
module.exports = { requirements, executionProtections, executionPolicyBinding, executionPolicy, applies };
