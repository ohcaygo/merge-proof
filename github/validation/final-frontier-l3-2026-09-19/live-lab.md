...
Update: the subsequent [development-App resume](app-resume.md) closes B05 delivery and B07 scoped reconciliation with unchanged installation scope and permissions. The later [second-account resume](reviewer-resume.md) validates approval and dismissal behavior, with the same-person limitation explicit. The [B03 permission-loss resume](permission-resume.md) also passed under explicit authorization, restoring Write access. This document preserves the initial run and its original blocked results.
