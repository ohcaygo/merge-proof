...
**NOT_PROVEN** means: sufficient current positive evidence could not be established. Insufficient, unavailable, ambiguous, stale, unsupported or otherwise unprovable evidence remains NOT_PROVEN. The receipt names the missing binding or field and next action. NOT_PROVEN never means the code is wrong.

**FAIL** means: current, sufficiently bound evidence demonstrates an unmet required condition. Collection errors, missing history, exceeded budgets, malformed data and other uncertainty yield NOT_PROVEN, never FAIL. This definition follows Ryan’s explicit implementation decision on 2026-09-19 and supersedes the conflicting research wording. VERIFIED requires sufficient current positive evidence.
